...
const SongListInfo: FC<Props> = props => {
    const {
        history,
        match,
        dispatch,
    ...
    const [songListDetail, setSongListDetail] = useState<any>({})
    ...
    useEffect(() => {
        ...
        if (songListId === '000000') {
            ...
            switch (tabSub) {
                case 'NETEASE': {
                    getNetSongListDetail(songListId).then(async (res: { playlist: any }) => {
                        ...
                        const ids: any[] = []
                        let newSongList: any[] = []
                        newSongListDetail.trackIds.forEach((item: { id: any }) => {
                            ids.unshift(item.id)
                        })
                        ...
                        newSongListDetail.songList.forEach((item: { title: any; name: any; singer: any; ar: { name: any }[]; description: any; al: { name: any; picUrl: any }; picUrl: any; time: number; dt: any }) => {
                ...
                case 'TENCENT': {
                    getTencentSongListDetail(songListId).then((res: { response: { cdlist: any[] } }) => {
                        ...
                        newSongListDetail.songlist.forEach((item: { title: any; name: any; singerList: any; singer: { name: any }[]; description: any; album: { name: any; mid: string }; picUrl: string; time: any; interval: any }) => {
                            item.title = item.name
                ...
                case 'XIAMI':
                    getXiamiSongListDetail().then((res: any) => {
    ...
    useEffect(() => {
        const isLove = () => {
            ...
            return loveSongList.some((item: any) => {
    ...
    const isLove = () => {
        const loveSongList = getLoveSongList()
        return loveSongList.some((item: { id: any }) => {
            if (String(item.id) === songListId) {
                return true
            } else {
                return false
        ...
        return (
            <div className={styles.songList}>
                {songListDetail.songList ?
                    songListDetail.songList.map((item: any, index: number) => (
                        <SongItem
                            tab={tabSub}
                            songDetail={item}
...
export default connect()(SongListInfo)
...
const PlayerHome: FC<Props> = props => {
    const {
        history,
        dispatch,
        player,
    ...
    useEffect((() => {
        if (playSong && JSON.stringify(playSong) !== '{}') {
            switch (platform) {
                case 'NETEASE':
                    getNetSongLyric(playSong.id).then((res: { nolyric: any; lrc: { lyric: any } }) => {
                        if (res.nolyric || !res.lrc.lyric) {
                            setLyric({})
                        } else {
                            const newLyricObj = new Lyric(res.lrc.lyric, changeCurrentLyricNum)
                            // newLyricObj.play()
                    ...
                    break
                case 'TENCENT':
                    getTencentSongLyric(playSong.mid).then((res: { response: { lyric: any } }) => {
                        const newLyricObj = new Lyric(res.response.lyric, changeCurrentLyricNum)
                        setLyric(newLyricObj)
                    })
                    break
                case 'XIAMI':
            ...
            const isLove = loveSong.some((item: { id: any }) => {
    ...
    const getPlayUrl = async (song: { id: string; mid: string }) => {
        switch (platform) {
            case 'NETEASE':
                await getNetSongDetail(song.id).then((res: { data: { url: any }[] }) => {
                ...
                break
            case 'TENCENT':
                await getTencentSongDetail(song.mid).then((res: { response: { req_0: { data: { midurlinfo: { purl: string }[] } } } }) => {
    ...
    const handleClickProgessBar = (e: React.MouseEvent<HTMLDivElement, MouseEvent>) => {
        e.stopPropagation()
    ...
    const handleDotTouchEnd = (e: React.TouchEvent<HTMLDivElement>) => {
        if (!playSong || player.playUrl.length === 0 || JSON.stringify(playSong) === '{}') {
            setIsPlay(false)
            return (Toast.info('没有播放地址'))
        } else {
    ...
    const handleShowPlayList = (e: React.MouseEvent<HTMLElement, MouseEvent>) => {
        if (!playSong || player.playUrl.length === 0 || JSON.stringify(playSong) === '{}') {
            Toast.info('播放列表为空')
        } else {
            e.stopPropagation()
    ...
    const nextSong = (e: React.MouseEvent<HTMLDivElement, MouseEvent> | React.SyntheticEvent<HTMLAudioElement, Event>) => {
        e.stopPropagation()
        isFirstLoad.current = false
    ...
    const prevSong = (e: React.MouseEvent<HTMLDivElement, MouseEvent>) => {
        e.stopPropagation()
        isFirstLoad.current = false
        if (!playSong || player.playUrl.length === 0 || JSON.stringify(playSong) === '{}') {
            Toast.info('播放列表为空')
    ...
    const isLove = () => {
        const loveSong = getLoveSong()
        return loveSong.some((item: { id: any }) => {
            if (item.id === playSong.id) {
                return true
            } else {
                return false
            ...
            <div className={styles.picAndLyricWrapper}>
                {showLyric ?
                    <div className={styles.lyricWrapeer} onClick={() => {
                        ...
                        <div className={classNames(styles.linWrapper, { [styles.noLyric]: JSON.stringify(lyric) === '{}' })}>
        ...
        return (
            <div className={styles.bottomWrapper}>
                <div className={styles.iconBar}>
                    <div className={classNames(styles.iconWrapper, { [styles.isLove]: isLoveSong })} onClick={() => { changeLoveState() }}>
                        {isLoveSong ?
        ...
        <Fragment>
            {player.showMini ?
                <MiniPlay
                    ...
                    playSong={playSong}
                    isPlay={isPlay}
                    showPlayList={(e: any) => { handleShowPlayList(e) }}

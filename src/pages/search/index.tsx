...
const Search: FC<Props> = props => {
    ...
    // 获取网易云数据
    const getNetData = () => {
        getNetHotSearch().then((res: { data: any }) => {
            const newHotSearch = res.data
            newHotSearch.forEach((item: { id: any; title: any; searchWord: any; description: any; content: any; num: any; score: any }, index: any) => {
                item.id = index
    ...
    const getTencentData = () => {
        getTencentHotSearch().then((res: { response: { data: { hotkey: any } } }) => {
            const newHotSearch = res.response.data.hotkey
            newHotSearch.forEach((item: { id: any; title: any; k: any; description: string; num: any; n: any }, index: any) => {
                item.id = index
                item.title = item.k
                item.description = ''
                item.num = item.n
    ...
    // 获取虾米音乐数据
    const getXiamiData = () => {
        getXiamiHotSearch().then((res: { data: React.SetStateAction<never[]> }) => {
        ...
        if (!value && searchValue === '') {
            ...
            switch (tab) {
                case 'NETEASE': {
                    const ids: any[] = []
                    let newResult: any = {}
                    await getNetSearchResult(newValue).then((res: { result: {} }) => {
                        newResult = { ...res.result }
                        newResult.songs.forEach((item: { id: any }) => {
                            ids.push(item.id)
                        })
                    })
                    await getNetSongDetailData(ids.join(',')).then((res: { songs: any }) => {
                        const newSongList: any = [...res.songs]
                        newResult.songs.forEach((item: any, index: string | number) => {
                            Object.assign(item, newSongList[index])
                        })
                    })
                    newResult.songs.forEach((item: { title: any; name: any; singer: any; artists: { name: any }[]; description: any; album: { name: any }; time: number; duration: any; picUrl: any; al: { picUrl: any } }) => {
                        item.title = item.name
                        item.singer = item.artists[0].name
                        item.description = item.album.name
                        item.time = Number.parseInt(String((item.duration) / 1000))
                ...
                case 'TENCENT':
                    await getTencentSearchResult(newValue).then((res: { response: { data: { song: any } } }) => {
                        const newResult = { ...res.response.data.song }
                        newResult.songs = newResult.list
                        newResult.songs.forEach((item: { title: any; name: any; singerList: any; singer: { name: any }[]; description: any; album: { name: any; mid: string }; picUrl: string; time: any; interval: any }) => {
                            item.title = item.name
                    ...
                    break
                case 'XIAMI':
                    await getXiamiSearchResult().then((res: { data: React.SetStateAction<{}> }) => {
                ...
                {JSON.stringify(searchResult) !== '{}' ?
                    ...
                    播放全部
                        ...
                        <div className={styles.songWrapper}>
                            {searchResult.songs && searchResult.songs.map((item: any, index: number) => (
    ...
    const renderHot = () => {
        return (
            <div className={styles.hotWrapper}>
                ...
                {hotSearch ?
                    <div className={styles.itemWrapper}>
                        {hotSearch && hotSearch.map((item: any, index: number) => (
